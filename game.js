...
body{background:#03070f;font-family:'Orbitron',monospace;overflow:hidden;width:100vw;height:100vh;}

/* Canvas NEVER blocks clicks — pointer-events:none always */
#cv{
  position:fixed;top:0;left:0;
  width:100%;height:100%;
  display:block;
  pointer-events:none; /* KEY FIX: canvas never steals clicks/touches */
  z-index:1;
}

/* Transparent overlay that captures swipe ONLY during game */
#touch-layer{
  position:fixed;inset:0;z-index:2;
  display:none; /* shown only when game is running */
  touch-action:none;
}
...
.hud-logo{font-weight:900;font-size:clamp(.5rem,2.2vw,.8rem);letter-spacing:.22em;color:#00e5ff;}
.hud-logo b{color:#fff;}
.hud-mid{display:flex;gap:1.2em;}
.hud-item{text-align:center;}
.hud-item .lbl{font-family:'Share Tech Mono',monospace;font-size:clamp(.36rem,1.4vw,.52rem);color:rgba(0,229,255,.4);letter-spacing:.12em;}
.hud-item .val{font-size:clamp(.65rem,2.5vw,.92rem);font-weight:700;}
.cy{color:#00e5ff;}.ye{color:#ffd60a;}.or{color:#ff6b35;}
.lives{font-size:clamp(.8rem,2.8vw,1rem);}

/* Logo */
.logo{font-size:clamp(1.8rem,8vw,4.5rem);font-weight:900;letter-spacing:.22em;color:#fff;
  text-shadow:0 0 30px #00e5ff,0 0 60px rgba(0,229,255,.4);margin-bottom:.15em;
  animation:lp 3s ease-in-out infinite;}
.logo span{color:#00e5ff;}
@keyframes lp{0%,100%{text-shadow:0 0 30px #00e5ff,0 0 60px rgba(0,229,255,.35);}
  50%{text-shadow:0 0 50px #00e5ff,0 0 100px rgba(0,229,255,.6);}}
.sub{font-family:'Share Tech Mono',monospace;font-size:clamp(.5rem,2.5vw,.78rem);
  letter-spacing:.35em;color:rgba(0,229,255,.45);margin-bottom:clamp(1em,4vw,2em);}

/* Card */
.card{background:rgba(3,7,15,.92);border:1px solid rgba(0,229,255,.25);border-radius:14px;
  padding:clamp(1.2em,4vw,2em) clamp(1.4em,5vw,2.5em);width:100%;max-width:420px;
  backdrop-filter:blur(20px);box-shadow:0 0 50px rgba(0,229,255,.08);}
.card h2{font-size:clamp(.7rem,3vw,.95rem);letter-spacing:.15em;color:#fff;margin-bottom:1em;font-weight:700;}
.hint-row{display:flex;gap:.6em;align-items:flex-start;
  font-family:'Share Tech Mono',monospace;font-size:clamp(.58rem,2.2vw,.72rem);
  color:rgba(255,255,255,.5);margin-bottom:.4em;line-height:1.4;}
.divider{height:1px;background:rgba(0,229,255,.2);margin:1em 0;}
.best-line{font-family:'Share Tech Mono',monospace;font-size:clamp(.58rem,2vw,.7rem);
  color:rgba(0,229,255,.45);letter-spacing:.1em;text-align:center;margin-bottom:1.2em;}
.best-line b{color:#ffd60a;}

/* BUTTON — works everywhere */
.play-btn{
  display:block;width:100%;
  padding:clamp(.85em,3vw,1.05em);
  background:#00e5ff;color:#03070f;
  border:none;border-radius:8px;
  font-family:'Orbitron',monospace;font-weight:900;
  font-size:clamp(.75rem,3vw,1rem);letter-spacing:.2em;
  cursor:pointer;
  box-shadow:0 0 25px rgba(0,229,255,.5);
  -webkit-tap-highlight-color:rgba(0,229,255,.25);
  touch-action:manipulation;
  -webkit-appearance:none;
  user-select:none;
}
.play-btn:hover{box-shadow:0 0 45px rgba(0,229,255,.75);}
.play-btn:active{opacity:.85;transform:scale(.97);}

/* Game over stats */
.score-num{font-size:clamp(2.5rem,10vw,4rem);font-weight:900;color:#ffd60a;
  text-shadow:0 0 25px rgba(255,214,10,.5);line-height:1;margin:.15em 0;}
.stats-grid{display:grid;grid-template-columns:1fr 1fr;gap:.6em;margin:1em 0;}
.stat-box{background:rgba(0,229,255,.05);border:1px solid rgba(0,229,255,.12);
  border-radius:8px;padding:.6em .4em;text-align:center;}
.stat-box .sl{font-family:'Share Tech Mono',monospace;font-size:clamp(.42rem,1.6vw,.56rem);
  color:rgba(0,229,255,.4);letter-spacing:.1em;}
.stat-box .sv{font-size:clamp(.75rem,3vw,1rem);font-weight:700;color:#fff;margin-top:.15em;}
.new-rec{display:none;align-items:center;justify-content:center;gap:.4em;
  background:rgba(255,214,10,.1);border:1px solid rgba(255,214,10,.3);
  border-radius:20px;padding:.3em .9em;margin:.5em auto;
  font-family:'Share Tech Mono',monospace;font-size:clamp(.52rem,2vw,.68rem);color:#ffd60a;}

/* Flash */
#flash{position:fixed;inset:0;z-index:30;background:rgba(255,107,53,.25);
  pointer-events:none;opacity:0;transition:opacity .06s;}
#flash.on{opacity:1;}

/* Level banner */
#lvl-banner{position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);
  z-index:25;pointer-events:none;text-align:center;opacity:0;transition:opacity .35s;}
#lvl-banner.show{opacity:1;}
.lvl-num{font-size:clamp(2.5rem,12vw,6rem);font-weight:900;
  background:linear-gradient(135deg,#00e5ff,#ffd60a);
  -webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;
  filter:drop-shadow(0 0 20px rgba(0,229,255,.5));}
.lvl-sub{font-family:'Share Tech Mono',monospace;font-size:clamp(.55rem,2.5vw,.85rem);
  color:rgba(255,255,255,.55);letter-spacing:.35em;}

/* Combo */
#combo-pop{position:fixed;top:42%;left:50%;transform:translate(-50%,-50%) scale(0);
  z-index:25;pointer-events:none;font-size:clamp(1.3rem,6vw,3rem);font-weight:900;
  color:#ffd60a;text-shadow:0 0 20px #ffd60a;opacity:0;
  transition:transform .18s cubic-bezier(.34,1.56,.64,1),opacity .3s;}
#combo-pop.show{transform:translate(-50%,-70%) scale(1);opacity:1;}
...
<!-- Touch layer — shown only during game to capture swipes -->
...
<div id="lvl-banner"><div class="lvl-num" id="lvl-num-text"></div><div class="lvl-sub">НОВИЙ РІВЕНЬ</div></div>
...
<div id="screen-start">
  ...
  <div class="card">
    ...
    <div class="hint-row"><span>👆</span><span>На телефоні — тягни пальцем куди летіти</span></div>
    <div class="hint-row"><span>☄️</span><span>Уникай комет, астероїдів і чорних дір</span></div>
    ...
    <button class="play-btn" id="btn-start" type="button">▶ &nbsp;ЗАПУСТИТИ РАКЕТУ</button>
  </div>
</div>
...
<div id="screen-over">
  ...
  <div class="card">
    ...
    <button class="play-btn" id="btn-restart" type="button">↺ &nbsp;ЛЕТІТИ ЗНОВУ</button>
  </div>
</div>
...
// ── TOUCH — on the touch-layer div, NOT on canvas or screens ──
let tStart=null, tDelta={x:0,y:0};
touchLayer.addEventListener('touchstart',e=>{
  e.preventDefault();
  const t=e.touches[0]; tStart={x:t.clientX,y:t.clientY}; tDelta={x:0,y:0};
},{passive:false});
touchLayer.addEventListener('touchmove',e=>{
  e.preventDefault();
  if(!tStart) return;
  const t=e.touches[0];
  tDelta={x:(t.clientX-tStart.x)/28, y:(t.clientY-tStart.y)/28};
  tStart={x:t.clientX,y:t.clientY};
},{passive:false});
touchLayer.addEventListener('touchend',()=>{tStart=null;tDelta={x:0,y:0};});
...
function initStars(){
  stars=Array.from({length:240},()=>({
    ...
    r:Math.random()*1.6+.15, a:Math.random()*.8+.2,
    spd:Math.random()*2.5+.4, layer:Math.floor(Math.random()*3)
  }));
}
function drawStars(sp){
  stars.forEach(s=>{
    const v=s.spd*sp*(s.layer===0?.35:s.layer===1?.8:1.5);
    s.x-=v; if(s.x<-2){s.x=W+2;s.y=Math.random()*H;}
    if(s.layer===2&&sp>3){
      ...
      ctx.fillStyle=g; ctx.fillRect(s.x,s.y-s.r*.4,v*4,s.r*.8);
    }
...
function initNebs(){
  nebs=Array.from({length:5},()=>({
    x:Math.random()*W, y:Math.random()*H,
    r:Math.random()*200+100, hue:Math.random()*360,
    a:Math.random()*.06+.025, spd:.12+Math.random()*.18
  }));
}
function drawNebs(sp){
  nebs.forEach(n=>{
    n.x-=n.spd*sp; if(n.x+n.r<0){n.x=W+n.r;n.y=Math.random()*H;}
    const g=ctx.createRadialGradient(n.x,n.y,0,n.x,n.y,n.r);
    g.addColorStop(0,`hsla(${n.hue},70%,35%,${n.a})`); g.addColorStop(1,'transparent');
...
function drawRocket(x,y,ft){
  ...
  fg.addColorStop(0,'rgba(255,210,60,.95)'); fg.addColorStop(.45,'rgba(255,100,20,.7)'); fg.addColorStop(1,'rgba(255,40,0,0)');
  ctx.fillStyle=fg;
  ctx.beginPath(); ctx.moveTo(-W2*.35,H2*.18); ctx.quadraticCurveTo(-W2*.5,H2*.22+fl*.5,0,H2*.22+fl); ctx.quadraticCurveTo(W2*.5,H2*.22+fl*.5,W2*.35,H2*.18); ctx.fill();
  let fi=ctx.createLinearGradient(0,H2*.2,0,H2*.2+fl*.6);
  fi.addColorStop(0,'rgba(255,255,200,1)'); fi.addColorStop(1,'rgba(255,180,40,0)');
  ctx.fillStyle=fi;
  ctx.beginPath(); ctx.moveTo(-W2*.13,H2*.2); ctx.quadraticCurveTo(-W2*.18,H2*.2+fl*.3,0,H2*.2+fl*.62); ctx.quadraticCurveTo(W2*.18,H2*.2+fl*.3,W2*.13,H2*.2); ctx.fill();
  // body
  let bg2=ctx.createLinearGradient(-W2*.5,0,W2*.5,0);
  bg2.addColorStop(0,'#0d2a4a'); bg2.addColorStop(.3,'#1976d2'); bg2.addColorStop(.5,'#82cbf5'); bg2.addColorStop(.7,'#1976d2'); bg2.addColorStop(1,'#0a1e38');
  ctx.fillStyle=bg2;
  ctx.beginPath(); ctx.moveTo(0,-H2*.5); ctx.quadraticCurveTo(W2*.48,-H2*.12,W2*.44,H2*.08); ctx.lineTo(W2*.36,H2*.24); ctx.lineTo(-W2*.36,H2*.24); ctx.lineTo(-W2*.44,H2*.08); ctx.quadraticCurveTo(-W2*.48,-H2*.12,0,-H2*.5); ctx.fill();
  // nose glow
  ...
  // cockpit
  ctx.fillStyle='rgba(0,229,255,.12)'; ctx.strokeStyle='rgba(0,229,255,.55)'; ctx.lineWidth=1.2;
  ctx.beginPath(); ctx.ellipse(0,-H2*.13,W2*.2,H2*.11,0,0,Math.PI*2); ctx.fill(); ctx.stroke();
  ...
  // outline glow
  ctx.shadowColor='rgba(0,229,255,.5)'; ctx.shadowBlur=10; ctx.strokeStyle='rgba(0,229,255,.22)'; ctx.lineWidth=1;
  ctx.beginPath(); ctx.moveTo(0,-H2*.5); ctx.quadraticCurveTo(W2*.48,-H2*.12,W2*.44,H2*.08); ctx.lineTo(W2*.36,H2*.24); ctx.lineTo(-W2*.36,H2*.24); ctx.lineTo(-W2*.44,H2*.08); ctx.quadraticCurveTo(-W2*.48,-H2*.12,0,-H2*.5); ctx.shadowBlur=0; ctx.stroke();
  ctx.restore();
}
...
function drawComet(x,y,r,rot){
  ...
  ctx.fillStyle=tg; ctx.beginPath(); ctx.moveTo(r*.3,-r*.22); ctx.lineTo(r*.3+tl,0); ctx.lineTo(r*.3,r*.22); ctx.closePath(); ctx.fill();
...
function drawAsteroid(x,y,r,rot,verts){
  ...
  ctx.fillStyle=ag; ctx.fill(); ctx.strokeStyle='rgba(200,180,150,.2)'; ctx.lineWidth=1; ctx.stroke();
...
function drawStar(x,y,r,t){
  ctx.save(); ctx.translate(x,y);
  const sc=.82+Math.sin(t*.09)*.18; ctx.scale(sc,sc);
  ...
  ctx.fillStyle='#ffe050'; ctx.beginPath();
  for(let i=0;i<5;i++){
...
function drawShieldPU(x,y,r,t){
  ...
  ctx.beginPath(); ctx.moveTo(0,-r*.7); ctx.lineTo(r*.55,-r*.35); ctx.lineTo(r*.55,r*.1);
  ctx.quadraticCurveTo(r*.3,r*.65,0,r*.8); ctx.quadraticCurveTo(-r*.3,r*.65,-r*.55,r*.1);
...
function drawParts(){
  ...
  parts.forEach(p=>{
    ...
    ctx.save();ctx.globalAlpha=p.life;ctx.beginPath();ctx.arc(p.x,p.y,p.r*p.life,0,Math.PI*2);ctx.fillStyle=p.color;ctx.fill();ctx.restore();
  });
}

let pops=[];
function popText(x,y,text,color='#ffd60a',size=20){pops.push({x,y,text,color,size,life:1,vy:-1.7});}
function drawPops(){
  pops=pops.filter(p=>p.life>0);
  pops.forEach(p=>{
    p.y+=p.vy;p.life-=.018;
    ctx.save();ctx.globalAlpha=p.life;ctx.font=`bold ${p.size}px Orbitron,monospace`;
    ctx.textAlign='center';ctx.fillStyle=p.color;ctx.shadowColor=p.color;ctx.shadowBlur=10;
...
// ════════════════════════
let trail=[];
function updateTrail(x,y){trail.unshift({x,y});if(trail.length>20)trail.pop();}
function drawTrail(sh){
  trail.forEach((pt,i)=>{
    const a=(1-i/trail.length)*(sh?.55:.4),r=(1-i/trail.length)*10+2;
    ctx.beginPath();ctx.arc(pt.x,pt.y,r,0,Math.PI*2);
    ctx.fillStyle=sh?`rgba(0,229,255,${a})`:`rgba(0,180,220,${a})`;ctx.fill();
  });
}
...
let P,obs,coll,score,lives,level,combo,maxCombo,speed,frame,inv,shieldOn,shieldTimer,dist;
let spawnT,collectT,scoreAcc,distAcc,lastT;

const HUD_H=52;
const OBS_T=[
  {kind:'comet',r:15,pts:5,spd:1.2},{kind:'comet',r:21,pts:8,spd:.88},
  {kind:'asteroid',r:19,pts:10,spd:.72},{kind:'asteroid',r:27,pts:15,spd:.56},
  {kind:'bhole',r:17,pts:25,spd:.58}
];
const COL_T=[
  {kind:'star',r:13,pts:50,spd:1.1},{kind:'star',r:17,pts:80,spd:.9},
...
function spawnObs(){
  const t=OBS_T[Math.floor(Math.random()*OBS_T.length)];
...
function spawnColl(){
  const t=COL_T[Math.floor(Math.random()*COL_T.length)];
  const spd=speed*(Math.random()*.4+.85);
  coll.push({...t,x:W+55,y:65+Math.random()*(H-130),vx:-spd,vy:(Math.random()-.5)*spd*.3,rot:0,t:0});
}
...
function setCombo(v){
  combo=Math.min(v,12);maxCombo=Math.max(maxCombo,combo);
  $('h-combo').textContent='x'+combo;
  if(combo>=3){
    const el=$('combo-pop');el.textContent='COMBO x'+combo+'!';el.classList.add('show');
    clearTimeout(el._t);el._t=setTimeout(()=>el.classList.remove('show'),850);
  }
}

function showLevel(){
  $('lvl-num-text').textContent='РІВЕНЬ '+level;
  const el=$('lvl-banner');el.classList.add('show');
  setTimeout(()=>el.classList.remove('show'),1700);
  spawnParts(W*.5,H*.5,'#00e5ff',18,6);spawnParts(W*.5,H*.5,'#ffd60a',18,6);
}
...
function loop(ts){
  ...
  if(newLv>level){level=newLv;showLevel();}
  ...
  // move player
  const SP=5.8;
  if(keys['ArrowUp']  ||keys['w']||keys['W']) P.vy-=1.1*dt;
  if(keys['ArrowDown']||keys['s']||keys['S']) P.vy+=1.1*dt;
  if(keys['ArrowLeft']||keys['a']||keys['A']) P.vx-=1.1*dt;
  if(keys['ArrowRight']||keys['d']||keys['D'])P.vx+=1.1*dt;
  P.vx+=tDelta.x*1.6; P.vy+=tDelta.y*1.6; tDelta={x:0,y:0};
  ...
  coll.forEach(o=>{o.kind==='star'?drawStar(o.x,o.y,o.r,o.t):drawShieldPU(o.x,o.y,o.r,o.t);});
  obs.forEach(o=>{
  ...
  if(shieldOn){
    const p=.55+Math.sin(frame*.15)*.45;
    ctx.save();ctx.strokeStyle=`rgba(0,229,255,${p})`;ctx.lineWidth=2;
  ...
  if(!(inv&&Math.floor(frame/5)%2===0)) drawRocket(P.x,P.y,P.ft);
  ...
  if(!inv){
    for(let i=coll.length-1;i>=0;i--){
      ...
      if(circHit(P.x,P.y,P.r*.85,o.x,o.y,o.r)){
        coll.splice(i,1);
        if(o.kind==='shield'){shieldOn=true;shieldTimer=280;spawnParts(P.x,P.y,'#00e5ff',14,4);popText(P.x,P.y-35,'ЩІИТ!','#00e5ff',18);}
        else{const pts=o.pts*combo;score+=pts;$('h-score').textContent=score;setCombo(combo+1);burst(o.x,o.y);popText(o.x,o.y-22,'+'+pts,'#ffd60a');}
      }
    }
    for(let i=obs.length-1;i>=0;i--){
      const o=obs[i];
      if(circHit(P.x,P.y,P.r*.78,o.x,o.y,o.r)){
        if(shieldOn){shieldOn=false;shieldTimer=0;spawnParts(o.x,o.y,'#00e5ff',16,5);obs.splice(i,1);popText(P.x,P.y-35,'ЗАХИСТ!','#00e5ff',18);}
        else{
          lives--;$('h-lives').textContent='❤️'.repeat(Math.max(0,lives));
          flashHit();spawnParts(P.x,P.y,'#ff6b35',18,5);obs.splice(i,1);setCombo(1);
...
function startGame(){
  ...
  inv=false;shieldOn=false;shieldTimer=0;dist=0;
  spawnT=0;collectT=0;scoreAcc=0;distAcc=0;
  ...
  touchLayer.style.display='block'; // enable swipe capture
...
function endGame(){
  ...
  $('go-newrec').style.display=isNew?'flex':'none';
